...
session_start();
$pageTitle = 'Dashboard';
$is_dashboard = true; // Habilita el menú lateral
...
try {
    ...
    $rol_usuario = $_SESSION['id_rol'] ?? 2; // Por defecto técnico
    $es_admin = ($rol_usuario == 1);

    // 1. NÚMERO DE PROYECTOS CREADOS
    if ($es_admin) {
        // Admin ve todos los proyectos
        $totalProyectos = $pdo->query("SELECT COUNT(*) FROM proyectos WHERE activo = 1")->fetchColumn() ?? 0;
    } else {
        // Técnicos solo ven proyectos creados por ellos
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM proyectos WHERE creado_por = ? AND activo = 1");
        $stmt->execute([$usuario_actual]);
        $totalProyectos = $stmt->fetchColumn() ?? 0;
    }

    // 2. BENEFICIARIOS DE PROYECTOS COMPLETADOS
    $totalBeneficiariosCompletados = $pdo->query("
    ...
    $inversionCompletados = $pdo->query("
    ...
    $inversionEsperada = $inversionCompletados + $inversionPendiente;

    // 6. PORCENTAJE DE PROYECTOS ENTREGADOS
    $totalAsignaciones = $pdo->query("SELECT COUNT(*) FROM proyecto_asignacion")->fetchColumn() ?? 0;
    $proyectosCompletados = $pdo->query("
        SELECT COUNT(*) 
        FROM asignacion_entregas_resumen 
        WHERE completado = 1
    ")->fetchColumn() ?? 0;
    
    $porcentajeEntregados = $totalAsignaciones > 0 ? ($proyectosCompletados / $totalAsignaciones) * 100 : 0;

    // 7. PROYECTOS EN PROGRESO
    $proyectosEnProgreso = $pdo->query("
        SELECT COUNT(*) 
        FROM proyecto_asignacion pa
    ...
    $proyectosCompletadosRecientes = $pdo->query("
        SELECT 
            pa.id_asignacion,
            pa.comunidad_nombre,
            pa.bloque,
    ...
    // Si hay un error en la consulta, establecer valores por defecto
    $totalProyectos = 0;
